...
namespace Assembly_OwnDLLs_Accounts
{
    ...
    public class Accounts_Management
    {
        ...
        public static int countFrozenAIDs(ref int count)
        {
            List<int> AIDlist = new List<int>();
            int itAID = 1;
            bool status = true;

            if (isAccountUnfrozen(itAID) == -1) status = false;

            while (status)
            {
                if (isAccountUnfrozen(itAID) == 0)
                {
                    AIDlist.Add(itAID);
                }
        ...
        static void Main(string[] args)
        {
            ...
            int tmpp = 0;
            Console.WriteLine(countFrozenAIDs(ref tmpp));
            Console.WriteLine("Number of Frozen Accounts {0}", tmpp);
            freezeAccount(1);
            Console.WriteLine(countFrozenAIDs(ref tmpp));
            Console.WriteLine("Number of Frozen Accounts {0}", tmpp);
        }
